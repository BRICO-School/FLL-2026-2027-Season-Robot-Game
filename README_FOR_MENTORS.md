...
- **機体ごとの校正表 `ROBOT_PROFILES`（2026-09-17 追加）**: `setup.py` がハブの名前で表を引き、その機体の車輪径・トレッドと、
  ...
  - なぜ要るか: ジャイロの目盛りはハブごとに違い、この本番機（Hub3）は校正前、`turn(90)` が本当は約 88.7° しか回っていなかった（ジャイロは気づかない）。
    公式の 3 軸校正＋ heading_correction で合わせる。Hub3 は **365.8（2026-09-18・暫定）**。
  - **ジャイロの 2 つの状態（2026-09-18 に判明）**: モーターで回すと、ジャイロは「1 周を約 365° と数える状態 A（多数）」と「約 360° と数える状態 B（少数・手で回したときと同じ）」を行き来する。間の値は出ず、分ける条件は未特定。
    校正表は多数派の A に合わせてある（365.8）。B のときは 90° あたり約 1.45° 回りすぎる。だから**要所で壁に当てて向きを取り直す**設計は変えない。
  - **1 台を表に足す手順（約 15 分）**: ①ハブを機体から外して `uv run pybricksdev run ble run_imu_calibrate_guided.py --name "<ハブ名>"`
    ...
    → ログの「1 周の読み」を約 365 の本（A）と約 360 の本（B）に分け、**A の本だけの平均**を `heading_correction` に書く（5 本以上）③書いたあと同じ確認を 3 回以上（A の本が 5 周で ±5° 以内なら合格）。
