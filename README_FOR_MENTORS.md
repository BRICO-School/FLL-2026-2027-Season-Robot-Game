...
- **PID ゲイン**: 今年は **Pybricks の既定値（7558, 0, 1889）をそのまま使う**（2026-09-17。KI を足しても良くならなかった）。昨年値 `distance_control` = (1000, 50, 10)、`heading_control` = (2000, 50, 100) は `setup_pid_control()` に比較用として残してあり、下の `DRIVE_SETTINGS = "old"` で有効になる。
- **new / old の切りかえ `DRIVE_SETTINGS`（2026-09-18 追加）**: `setup.py` 冒頭の `DRIVE_SETTINGS = "new"` を `"old"` にすると、速度・加速度・PID が昨年の値（直進 400・500 / 回転 240・850 / PID 昨年値）になる。寸法と校正表は今年のまま。run ファイルから `initialize_robot(drive_settings="old")` でも切りかえられ、起動時に「★ 走行設定: new/old」と表示される。
  - なぜあるか: 新旧の比較（replication-study Step 9・2026-09-18）で、直進の精度・回転のばらつき・時間は new、正方形の終点は old が良く、割れた。**開発中は両方でミッションを走らせて成功率で決める**（オーナー方針）。
- **機体ごとの校正表 `ROBOT_PROFILES`（2026-09-17 追加）**: `setup.py` がハブの名前で表を引き、その機体の車輪径・トレッドと、
  ...
  - なぜ要るか: ジャイロの目盛りはハブごとに違い、この本番機（Hub3）は校正前、`turn(90)` が本当は約 88.7° しか回っていなかった（ジャイロは気づかない）。
    公式の 3 軸校正＋ heading_correction で合わせる。Hub3 は **360.1（2026-09-19・暫定）**。2026-09-18 の 365.8 は取り下げ（下の項）。
  - **ジャイロの 2 つの状態（2026-09-18 に判明・2026-09-19 に条件が分かった・暫定）**: ジャイロは「1 周を約 365° と数える状態 A」と「約 360° と数える状態 B（手で回したときと同じ＝正確）」のどちらかになり、間の値は出ない。
    **状態 A は「プログラムを始めて約 1 秒以内に回り始めたとき」だけ出る。** 1 秒以上止まってから回れば 1 回目から B で、そのあとも B のまま
    （待たずに回した 4 本は 1 回目の 5 周だけ A・始めに 1 / 3 / 10 秒止まった 5 本は 1 回目から B・B の 1 周の読み 20 回 平均 360.11・SD 0.13）。しくみは未解明（速度に依らない。電池電圧・温度・振動でもない）。
    - **だから `initialize_robot()` は、機体が止まったまま 2 秒待ってから進む（`wait_gyro_settle()`・`GYRO_SETTLE_MS`・2026-09-19 追加）。** 機体を置いて手を離してから実行する。起動時に「✓ ジャイロの待ち完了」と出る。
      10 秒たっても止まらないと「⚠ ジャイロの待ち」の警告を出して進む（そのときは回転が約 1.4% ずれるかもしれない）。`selector.py` は `initialize_robot()` のあとボタン待ちで止まっているので、大会形式の走行は B になる見こみ（未測定）。
