...
## 19:43 — 動作前後のジャイロ情報や各軸の回転量を出力するログを追加した

### 変更概要

動作前後のジャイロ情報や各軸の回転量を出力するログを追加した

### 差分

```diff
diff --git a/run_gyro_motor_check.py b/run_gyro_motor_check.py
index b4e11a9..632127f 100644
--- a/run_gyro_motor_check.py
+++ b/run_gyro_motor_check.py
@@ -24,7 +24,7 @@
...
@@ -40,6 +40,19 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
     hub.imu.reset_heading(0)
     robot.reset()
     await wait(500)
...
@@ -48,6 +61,15 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
             await wait(300)
     await wait(1000)
     h1 = hub.imu.heading()
...
+    )
     ...
     print(
         "# MODE:",
@@ -89,7 +111,7 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
     print(
         "# モーターで回ったときのジャイロの 1 周の読み:",
         round(h2 / TURNS, 3),
-        "度（手で回したときは 361.8）",
