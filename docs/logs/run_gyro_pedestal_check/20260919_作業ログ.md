# 作業ログ: run_gyro_pedestal_check.py
...
## 10:44 — 台上で車輪を空転させ振動によるジャイロへの影響を検証する処理を追加した

### 変更概要

台上で車輪を空転させ振動によるジャイロへの影響を検証する処理を追加した

### 差分

```diff
diff --git a/run_gyro_pedestal_check.py b/run_gyro_pedestal_check.py
new file mode 100644
index 0000000..8411c07
--- /dev/null
+++ b/run_gyro_pedestal_check.py
@@ -0,0 +1,120 @@
+"""
...
## 11:00 — タイヤ接地時の回転検知による中止処理を追加し変数の初期化を修正した

### 変更概要

タイヤ接地時の回転検知による中止処理を追加し変数の初期化を修正した

### 差分

```diff
diff --git a/run_gyro_pedestal_check.py b/run_gyro_pedestal_check.py
index 7ff363b..5ae70ad 100644
--- a/run_gyro_pedestal_check.py
+++ b/run_gyro_pedestal_check.py
@@ -27,6 +27,7 @@ from setup import initialize_robot
...
+FLOOR_LIMIT_DEG = 45  # ジャイロの向きがこれより動いたら「タイヤが浮いていない」とみなして止める (度)。振動だけなら 5 周ぶんでも約 25 度まで
 ...
 async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
@@ -50,10 +51,11 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
     clock = StopWatch()
     samples = 0
     stationary_samples = 0  # 回転中に「静止」と判定された回数（ファームの向き推定の手がかり）
-    wz_min = 0  # Z 軸の角速度の最小・最大 (deg/s)。機体は回らないので、振れ幅＝振動の大きさ
...
+    h_max = 0.0
     while not drivebase.done():
         samples += 1
         if hub.imu.stationary():
@@ -64,6 +66,14 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
         h = hub.imu.heading()
         h_min = min(h_min, h)
         h_max = max(h_max, h)
