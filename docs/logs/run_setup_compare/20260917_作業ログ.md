...
## 13:24 — 新旧の走行設定による性能を比較検証するスクリプトを新規作成した。

### 変更概要

新旧の走行設定による性能を比較検証するスクリプトを新規作成した。
...
## 13:27 — 設定値の取得元を内部のDriveBaseに変更した。

### 変更概要

設定値の取得元を内部のDriveBaseに変更した。

### 差分

```diff
diff --git a/run_setup_compare.py b/run_setup_compare.py
index fcf40fb..a700276 100644
--- a/run_setup_compare.py
+++ b/run_setup_compare.py
@@ -84,7 +84,7 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
...
## 13:49 — 回転の回りすぎ打ち消し処理を一律で無効化しました

### 変更概要

回転の回りすぎ打ち消し処理を一律で無効化しました

### 差分

```diff
diff --git a/run_setup_compare.py b/run_setup_compare.py
index ff4b442..fbdd4a5 100644
--- a/run_setup_compare.py
+++ b/run_setup_compare.py
@@ -83,7 +83,7 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
     elif SETTINGS == "old_full":
         robot = build_old_full(hub, left_wheel, right_wheel)
...
## 13:56 — squareコースの1辺の長さを定数化し500mmに変更した。

### 変更概要

squareコースの1辺の長さを定数化し500mmに変更した。

### 差分

```diff
diff --git a/run_setup_compare.py b/run_setup_compare.py
index d7c6be4..bfac573 100644
--- a/run_setup_compare.py
+++ b/run_setup_compare.py
@@ -46,6 +46,8 @@ SETTINGS = "new"        # "new" / "old" / "old_full" / "new_short"
 COURSE = "straight"     # "straight" / "turn" / "square" / "mission"
 # ============================
...
+
 ...
 SHORT_STRAIGHT_ACC = None   # mm/s²  例: 1100
 SHORT_TURN_ACC = None       # deg/s² 例: 1200
@@ -86,6 +88,8 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
...
+        print("# square の 1 辺:", SQUARE_SIDE, "mm")
     ...
     print("# heading pid:", robot.heading_control().pid())
     print("# distance pid:", robot.distance_control().pid())
@@ -109,7 +113,7 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
             print("#  回転", i + 1, "回目のあと ジャイロ:", round(hub.imu.heading(), 2), "度")
     elif COURSE == "square":
         for _ in range(4):
-            await robot.straight(1000)
...
## 15:35 — 新設定時に旋回の回り足りなさ補正を適用するよう変更した

### 変更概要

新設定時に旋回の回り足りなさ補正を適用するよう変更した

### 差分

```diff
diff --git a/run_setup_compare.py b/run_setup_compare.py
index c9af45b..23a8d06 100644
--- a/run_setup_compare.py
+++ b/run_setup_compare.py
@@ -89,6 +89,8 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
...
+    print("# 回り足りなさの補正 =", fix)
     if COURSE == "square":
         print("# square の 1 辺:", SQUARE_SIDE, "mm")
     print("# settings:", robot._robot.settings())  # Robot.settings() は値を返さないので中の DriveBase から読む
@@ -109,14 +111,14 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
         await robot.straight(1000)          # 速度の引数は書かない（既定値の効きを見る）
     elif COURSE == "turn":
         for i in range(4):
-            await robot.turn(90, compensate=comp)
...
+            await robot.turn(90, compensate=comp, correct=fix)
         ...
         short = SETTINGS == "new_short"
@@ -131,7 +133,7 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
             if short:
                 await robot.turn(90, acceleration=SHORT_TURN_ACC)
             else:
-                await robot.turn(90, compensate=comp)
...
```diff
diff --git a/run_setup_compare.py b/run_setup_compare.py
index c15c123..1c096fb 100644
--- a/run_setup_compare.py
+++ b/run_setup_compare.py
@@ -90,7 +90,7 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
