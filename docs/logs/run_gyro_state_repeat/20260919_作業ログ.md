...
## 11:33 — 開始前の待機と静止判定処理および関連する引数を追加した

### 変更概要

開始前の待機と静止判定処理および関連する引数を追加した

### 差分

```diff
diff --git a/run_gyro_state_repeat.py b/run_gyro_state_repeat.py
index 65a519c..e5a0eb0 100644
--- a/run_gyro_state_repeat.py
+++ b/run_gyro_state_repeat.py
@@ -30,6 +30,7 @@ from setup import initialize_robot
 TURNS = 5
 REPEATS = 6
 DIRECTION = 1  # 1 = 右回り / -1 = 左回り
+START_WAIT_MS = 0  # 最初の 5 周の前に、さわらずに待つ時間 (ms)。「始めてすぐの 1 回だけ A」かを見るため
 ...
 AXLE_MM = 114.48
 # 加速・減速のところはタイヤのすべり方が違うので、比率は 1 周目の終わり〜4 周目の終わりで取る
@@ -68,6 +69,20 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
     print("# 前: tilt", hub.imu.tilt(), "/ 電池:", hub.battery.voltage(), "mV")
     drivebase = robot._robot  # 回転中も測るので、中の DriveBase を直接使う
...
+
     for i in range(REPEATS):
         h0 = hub.imu.heading()
         rz0 = hub.imu.rotation(Axis.Z)
@@ -127,8 +142,12 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
