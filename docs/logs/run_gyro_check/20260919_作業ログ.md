...
## 15:52 — 旋回動作でジャイロの状態を確認する検証プログラムを新規追加した

### 変更概要

旋回動作でジャイロの状態を確認する検証プログラムを新規追加した

### 差分

```diff
diff --git a/run_gyro_check.py b/run_gyro_check.py
new file mode 100644
index 0000000..b7580b0
--- /dev/null
+++ b/run_gyro_check.py
@@ -0,0 +1,86 @@
+"""
...
## 16:05 — ジャイロ確認が5回完了するごとに高い音を3回鳴らす処理を追加した。

### 変更概要

ジャイロ確認が5回完了するごとに高い音を3回鳴らす処理を追加した。

### 差分

```diff
diff --git a/run_gyro_check.py b/run_gyro_check.py
index 2d4c301..2cc4c7d 100644
--- a/run_gyro_check.py
+++ b/run_gyro_check.py
@@ -11,6 +11,7 @@ selector.py の 9 番に登録してある。競技と同じ流れ（initialize_
  3. 試したい扱い方をする（例: そのまま置いておく／持ち上げて振ってから置く／ホームへ運ぶまねをする）
  4. 機体の左側面を定規に当てて置き、フォースセンサーを押す → すぐ 5 周まわる
  5. 緑（ピー）になったら定規に当て直して手を離す → 青 → 1 周の読みが出る → セレクターに戻る（3 へ）
+ 6. 5 回そろうと高い音が 3 回鳴る → ハブの中央ボタンで止める（条件ごとに 1 回のプログラム＝ログ 1 本）
 ...
 出る行:  G, 回, 前の回が終わってから（1 回目はプログラムを始めてから）の時間 ms, 走り出すとき静止と判定されていたか,
         当て直しで動いた角度, 1 周の読み, 校正前の目盛りで
@@ -84,4 +85,8 @@ async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
         ", 電池",
         hub.battery.voltage(),
     )
