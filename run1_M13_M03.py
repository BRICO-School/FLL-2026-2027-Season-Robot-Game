...
async def run(hub, robot, left_wheel, right_wheel, left_lift, right_lift):
    ...
    await wait(300)  # 0.3秒待機

    # 回転設定を適用
    robot.settings(**turn_settings)
    await robot.turn(-30)  # 30度左回転
    await wait(700)  # 0.7秒待機

    # 回転設定を適用
    robot.settings(**turn_settings)
    await robot.turn(30)  # 30度右回転
...
async def sensor_logger_task():
    ...
    # ヘッダーを一度だけ表示
    print(
        "time,current_dist_mm,error_angle_deg,current_heading_deg,left_angle_deg,right_angle_deg,angle_diff_deg,left_speed_dps,right_speed_dps,speed_diff_dps,kp_dist,ki_dist,kd_dist,kp_head,ki_head,kd_head"
    )
