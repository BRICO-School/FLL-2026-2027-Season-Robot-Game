...
4. **フォースセンサー（Port.D）を押す** → プログラムが動く！
...
走り終わると、ターミナルで **せいこう / しっぱい を聞かれます**。キーを 1 つ押して Enter:
...
| `x` | しっぱい |
| `d` | とちゅうまで |
| `e` | ロボットがうごかなかった（つながらなかった等） |
| `s` | 記録しない（まちがえて走らせた等） |

つづけてメモを聞かれます（なければ Enter だけ）。
記録は `docs/trials/trials.csv` にたまり、走らせたときのコードも
`docs/trials/snapshots/` にコピーされます。
**何回走らせて何回せいこうしたか**が時系列でのこるので、プレゼンで
「改善のようす」を見せられます。しくみは [`docs/trial_log_spec.md`](docs/trial_log_spec.md)。

> 機構テストなど記録したくないときは、コマンドラインで `--no-trial` をつけるか、
> 環境変数 `TRIAL_LOG=0` を設定してください。
