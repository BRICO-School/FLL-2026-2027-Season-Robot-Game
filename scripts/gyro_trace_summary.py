...
def summarize_trace(path):
    ...
    for i, r in enumerate(table):
        ...
        if d_gyro > SEGMENT_DEG / 2 and d_enc:
            ...
            print(
                f"  {seg[0][1]:7.1f}〜{seg[-1][1]:7.1f}   {ratio:8.4f}           {d_rz / d_gyro:8.4f}"
    ...
    print(
        f"  全体の比率 {total:.4f} / 区間の最小〜最大 {min(ratios):.4f}〜{max(ratios):.4f}"
...
def summarize_pedestal(path):
    ...
    volt = find(text, r"# 電池: " + NUM)
    print(
        f"  {name}  動いた向き {moved} 度  静止判定 {still.group(1) + '/' + still.group(2) if still else '?'}"
...
def main():
    ...
    pedestal = sorted(
        p for p in glob.glob(os.path.join(LOGS, "run_gyro_pedestal_check", "*.log")) if key in p
    )
    if pedestal:
    ...
    results = []
    for p in sorted(
        p for p in glob.glob(os.path.join(LOGS, "run_gyro_state_trace", "*.log")) if key in p
    ):
        r = summarize_trace(p)
        if r:
            results.append(r)
    if results:
        ...
        for state in ("A", "B", "?"):
            ...
            if rs:
                print(
                    f"  状態 {state}: n={len(rs)}  平均 {statistics.mean(rs):.4f}"
